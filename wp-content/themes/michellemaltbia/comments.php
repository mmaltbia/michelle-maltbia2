...
// Do not delete these lines
    ...
    if (have_comments()) : // if there are comments 
        ?>
        <div id="comments" class="clearfix col-xs-12 col-sm-8 col-sm-offset-2">
            <?php if (!empty($comments_by_type['comment'])) : // if there are normal comments  ?>
                <h4 class="title"><?php comments_number(__('No Comments', 'framework'), __('Comment(1)', 'framework'), __('Comments(%)', 'framework')); ?></h4>
                <ol class="comments">
                    <?php wp_list_comments('type=comment&avatar_size=51&callback=kbcf_comment'); ?>
                </ol>
            <?php endif; ?>
            <?php
            /* ----------------------------------------------------------------------------------- */
...
if (comments_open()) :
    ?>
    <div id="respond-wrap" class="clearfix col-xs-12">
        <div class="row">
                    ...
                    <?php else : ?>
                        <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
                            <?php if (is_user_logged_in()) :
                                echo '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>'; else : ?>
                                <div class="row">
                            ...
                            <div class="row">
                                <div class="form-group">
                                    <div class="col-md-12">
                                        <input name="submit" type="submit" class="btn btn-primary btn-lg" id="submit" style="border-color: #fff;" tabindex="5" value="<?php _e('Submit your comment', 'framework') ?>" />
